...
def benchmark_fp4_paged_mqa_logits(
    ...
    hdr = (
        ...
        f"{'maxAtom':>7s} {'DSL+cf':>7s} {'DSL-cf':>7s} {'cf-spd':>7s} | "
        f"{'maxNa':>5s} {'DSL+cf':>7s} {'max_atom/max_na':>15s} | "
    ...
    for next_n in next_ns:
        for context_len in context_lens:
            for batch_size in batch_sizes:
                ...
                def _make_dsl_fn(t, schedule_meta, use_cfence_flag, data=data):
                    def dsl_fn(
                        t=t, schedule_meta=schedule_meta, data=data, use_cfence_flag=use_cfence_flag
                    ):
                        torch.ops.trtllm.cute_dsl_fp4_paged_mqa_logits(
                            ...
                            epi_dtype=epi_dtype,
                            output_dtype=output_dtype,
                            use_cfence=use_cfence_flag,
                        )

                    return dsl_fn

                # baseline (max_atom): both cfence on and off — matches the
                # existing cfen_perf logs convention.
                base_cf_us = (
                    _bench_kineto(
                        _make_dsl_fn(base_t, dsl_schedule_meta_base, True),
                ...
                base_nc_us = (
                    _bench_kineto(
                        _make_dsl_fn(base_t, dsl_schedule_meta_base, False),
                        "kernel_cutlass_kernel",
                        num_iterations,
                    )
                    * 1e6
                )
                cf_speedup = base_nc_us / base_cf_us  # >1 = cfence faster
                # experimental (max_na): cfence-on only; reuses baseline timing
                # if strategies don't diverge.
                if strats_diverge:
                    exp_cf_us = (
                        _bench_kineto(
                            _make_dsl_fn(exp_t, dsl_schedule_meta_exp, True),
                            "kernel_cutlass_kernel",
                            num_iterations,
                        )
                        * 1e6
                    ...
                    exp_cf_us = base_cf_us
                strat_speedup = base_cf_us / exp_cf_us  # >1 = max_na faster
                ...
                ratio_base_str = f"{dg_us / base_cf_us:14.3f}x" if dg_us else "         N/A  "
                ratio_exp_str = f"{dg_us / exp_cf_us:12.3f}x" if dg_us else "       N/A  "
                ...
                print(
                    ...
                    f"{base_lab:>7s} {base_cf_us:7.1f} {base_nc_us:7.1f} {cf_speedup:6.3f}x | "
                    f"{exp_lab:>5s} {exp_cf_us:7.1f} {strat_speedup:14.3f}x | "
